...
No aceite nativo PR128, a abertura passou a comprovar versão SQLCipher 4 e HMAC ativos, exigir zero linhas em `cipher_integrity_check` e um resultado estrutural único `ok` em `integrity_check`. São verificações distintas; a ausência de erros criptográficos não comprova uma build cifrada sozinha. Falha em abertura/verificação/migration fecha a conexão sem fallback, apagamento automático ou liberação da réplica.

`executarTransacaoProtegida` mantém a transação em conexão nativa separada (`useNewConnection`), mas prepara chave, verificações e chaves estrangeiras antes de `BEGIN IMMEDIATE`. A conexão principal não compartilha essa transação. Commit/rollback e fechamento são centralizados; todos os caminhos transacionais da réplica, inclusive migrations e limpeza pós-login/QR, usam esse método. Não utilizar diretamente o helper exclusivo do Expo: ele não herda a chave SQLCipher da conexão original.
