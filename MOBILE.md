...
Correção do aceite nativo PR128: a abertura comprova SQLCipher 4 e HMAC ativos. `cipher_integrity_check` íntegro retorna zero linhas, enquanto `integrity_check` estrutural retorna exatamente uma linha `ok`. Ambos antecedem migrations e uso da réplica. Ausência da cifra, chave incorreta, erro criptográfico/estrutural ou falha de migration fecha a conexão e bloqueia o acesso sem apagar automaticamente banco ou chave.

Cada transação usa uma conexão exclusiva preparada com a mesma chave do cofre, cifra/HMAC e chaves estrangeiras antes de `BEGIN IMMEDIATE`. O helper exclusivo padrão do Expo abre outra conexão sem herdar esses pragmas, portanto não pode ser chamado diretamente no banco cifrado. Migrações, snapshot, lote, rascunho/pendência transacional e limpeza reutilizam `executarTransacaoProtegida`; falha reverte e sempre fecha a conexão, sem incluir consultas concorrentes da conexão principal.
...
A folha de ações passa a resolver detalhes e permissões pelo backend. Cliente abre Detalhes; faturas exibem `Dados do ERP em tempo real` junto da cobertura declarada ou `ERP indisponível — nenhum snapshot usado`. Na PR 117, o resultado MK é `JANELA_LIMITADA` a um mês: folha e Detalhes mostram `período consultado`, nunca omitem essa limitação e dizem `Nenhuma fatura retornada no período consultado` em vez de sugerir ausência integral de débitos. O contrato mobile recusa sucesso sem cobertura e não recebe identificador externo da fatura. Desbloqueio e ordem de serviço exigem prévia e confirmação explícita, e toda execução recebe chave idempotente nova. Resultado ambíguo é apresentado como processamento/reconciliação, nunca como sucesso. Segunda via, Pix, conexão, WhatsApp Flow e nota permanecem indisponíveis até terem caso de uso autorizado ponta a ponta.
