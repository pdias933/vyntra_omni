...
| 128 | Aceite integrado, build nativa e publicação | `xhigh` | Builds Debug iOS/Android geradas; correção da abertura SQLCipher implementada | Web publicada e convergência API aprovadas; correção mobile disponível via Metro, aceite nativo completo pendente | Portões completos; regressão da abertura e ensaio SQLCipher nativo no Mac aprovados | Staging permanece `pr-128a-df35243`; correção somente mobile, sem novo deploy de servidor | iPhone instalado/login aberto; novo pareamento e fluxo completo pendentes; Samsung Android 16 instalado/iniciado, aguardando desbloqueio e confirmação visual |
