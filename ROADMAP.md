...
| 128 | Aceite integrado, build nativa e publicação | `xhigh` | Builds Debug iOS/Android, correções SQLCipher e faixa compacta de filtros implementadas | Web publicada e convergência API aprovadas; correções mobile disponíveis via Metro, aceite nativo completo pendente | Portões completos; regressão SQLCipher/transações e geometria declarada dos filtros aprovadas; validação visual física do ajuste pendente | Staging permanece `pr-128a-df35243`; alterações somente mobile, sem novo deploy de servidor | Usuário confirmou login Android e, após recarregar, iPhone. Ajuste visual dos filtros e lote operacional completo pendentes |
