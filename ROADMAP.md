...
PR124 permanece `EM ANDAMENTO`: engenharia, regressão automatizada e deploy concluídos. As development builds iOS/Android foram geradas em 7 de setembro; instalação e abertura do login no iPhone confirmadas, mas a homologação física completa de aparência e operação permanece pendente. Não há aprovação de distribuição nas lojas. As branches foram enviadas ao GitHub; a integração recusou a abertura automática dos objetos de pull request (403), e não houve merge presumido. Evidências e roteiro pendente: [design/VALIDACAO-TEMAS.md](design/VALIDACAO-TEMAS.md) e [builds nativas](docs/operacoes/PR-128-BUILDS.md).
...
| 128 | Aceite integrado, build nativa e publicação | `xhigh` | Builds Debug iOS/Android, correções SQLCipher e faixa compacta de filtros implementadas | Web publicada e convergência API aprovadas; correções mobile disponíveis via Metro, aceite nativo completo pendente | Portões completos; regressão SQLCipher/transações e geometria declarada dos filtros aprovadas; tamanho dos filtros confirmado pelo usuário no Android | Staging permanece `pr-128a-df35243`; alterações somente mobile, sem novo deploy de servidor | Login confirmado pelo usuário em Android/iPhone; tamanho dos filtros confirmado no Android. Lote operacional, acessibilidade e demais aceites físicos pendentes |
...
Fechamento de engenharia/staging em 6 de setembro de 2026: 484 testes API e 361 testes da raiz, tipos, lint, contratos, builds e exportações aprovados. A homologação publicada detectou e corrigiu o listener SSE nomeado; o segundo operador agora recebe a transferência sem reload. APK de desenvolvimento Android gerado; naquele momento, iOS aguardava CocoaPods. Branches PR125–128 enviadas ao GitHub, sem presumir criação/merge de objetos de pull request. MK permanece desativado, controles cadastral/financeiro com percentual zero e piloto desligado. Produção não foi promovida. As pendências físicas e o vínculo/encerramento do lote seguinte continuam explícitos.

Atualização de 7 de setembro: CocoaPods instalado em runtime isolado do usuário, build iOS assinada e instalada no iPhone, login aberto e confirmado pelo usuário. O primeiro pareamento físico revelou verificação incorreta do retorno de `cipher_integrity_check`, apesar da conclusão HTTP 200 no servidor. Corrigida a abertura com comprovação de SQLCipher/HMAC e integridade estrutural, sem remover criptografia, apagar dados ou alterar autenticação. Dez regressões adicionais e ensaio com a fonte SQLCipher 4.7.0 do Expo aprovados. A correção atende ambas as plataformas, mas ainda requer repetição física. Detalhes, limites e reprodução: [PR128 — builds e diagnóstico](docs/operacoes/PR-128-BUILDS.md). Effort da PR128 mantido `xhigh`; nenhuma conclusão funcional antecipada.

Complemento do aceite iOS: nova conclusão HTTP 200 seguida de falha local revelou que o helper transacional do Expo criava uma conexão sem herdar a chave. Todos os caminhos transacionais locais agora usam conexão exclusiva protegida antes de `BEGIN IMMEDIATE`. Quinze regressões de abertura/transação e ensaio nativo de conexão sem chave, commit/rollback e migrations reais v0→v5 aprovados. Código servido pelo Metro; confirmação física ainda pendente, sem novo deploy de servidor. PR128 permanece `EM ANDAMENTO`, Effort `xhigh`.

Correção de identificação da plataforma no aceite: o usuário esclareceu que o login bem-sucedido e os filtros muito altos ocorreram no **Android**, não no iPhone. A faixa horizontal herdava `flexGrow: 1` do ScrollView e disputava a altura com a lista. Ajustada para crescimento zero, conteúdo centralizado, altura mínima de toque e fonte dinâmica; lista ocupa o espaço restante. Regressão automatizada incluída, confirmação visual física pendente. O iPhone foi reiniciado com o código atual para nova tentativa, sem apagar dados; login iOS não está aprovado. Effort da PR128 mantido `xhigh`.

Confirmação posterior do usuário: após o reinício com código atual e novo pareamento, **login iOS bem-sucedido**. Login inicial confirmado em ambos os aparelhos; isso não encerra a homologação de resgate/nota/transferência, rede/revogação, acessibilidade ou ajuste visual dos filtros. Portões cumulativos do ajuste: 484 testes API por cache + 377 raiz, tipos, lint, contratos, builds/exportações aprovados.

Confirmação visual posterior em 7 de setembro: usuário confirmou o tamanho correto dos filtros no Android. Registro por relato, sem nova captura; não comprova aparência iOS, fonte ampliada ou Reduzir Movimento. PR128 permanece `EM ANDAMENTO`, Effort `xhigh`, até o aceite operacional e demais testes físicos.
