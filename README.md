...
`Conversa` é única por `Contato` e nunca recebe estado de encerramento. Contas WhatsApp diferentes registram participações na mesma conversa, com primeiro/último instante próprios; isso preserva a origem sem fragmentar a timeline. Modelo e operação estão em [docs/operacoes/PR-027.md](docs/operacoes/PR-027.md); não houve nova dependência, conforme [docs/dependencias/PR-027.md](docs/dependencias/PR-027.md).
