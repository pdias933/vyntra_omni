...
A PR 092 aplica `default deny` também aos Detalhes do Contato. O backend autoriza a fila antes de consultar o contato e testa separadamente cada projeção; esconder seção no navegador não concede nem revoga acesso. BSUID exige `VISUALIZAR_DADO_SENSIVEL`, telefone sai mascarado e identificadores externos do ERP nunca entram no DTO. Troca de contexto exige sessão atual, origem, CSRF, fila derivada no servidor, `ALTERAR_CONTEXTO_CLIENTE`, pertencimento exato e versão esperada. Financeiro e elegibilidade usam ERP em tempo real ou falham explicitamente; snapshot não participa da decisão. Desbloqueio e ordem de serviço exigem preparo, confirmação literal, nova autorização e serviço idempotente de domínio.
...
- cada conexão transacional reaplica a chave custodiada e habilita as chaves estrangeiras antes de iniciar a transação; pragmas da conexão principal não são presumidos na conexão separada. Falha nunca confirma parcialmente nem envolve consultas concorrentes na mesma transação;
