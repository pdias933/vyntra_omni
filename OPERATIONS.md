...
O release pode ser implantado sem credencial MK porque `MK_MODO=DESATIVADO` é o padrão. Nesse estado, a integração aparece como não configurada e toda consulta real falha fechada sem tentar rede. `CARACTERIZACAO` existe somente para a ferramenta controlada e não torna o provider disponível à API. `SOMENTE_LEITURA` exige arquivos secretos regulares com tamanho validado antes da leitura, origem HTTPS em allowlist e configuração de timeout, limite de concorrência, teto de resposta e circuit breaker dentro dos intervalos aprovados. Não existe código global de serviço em variável: o adapter fixa `6`, `8`, `9` e `22` nas rotas correspondentes, usa um token por chamada e recusa qualquer lista de autorização ampla ou divergente.
...
O deploy de staging da PR 117/117A mantém modo e controles desligados, além de `PILOTO_MK_REAL=false`. Esta entrega não acrescenta telemetria externa do MK; portanto nenhum rollout real pode ser iniciado até existir observabilidade sanitizada de código normalizado, duração e estado do circuito, sem URL, parâmetro, identificador ou payload. Antes de qualquer ensaio real futuro, rotacionar as credenciais consideradas comprometidas e usar um perfil exclusivo que autorize somente os quatro serviços de leitura necessários; o perfil de caracterização que anunciou 114 serviços é proibido. Produção permanece bloqueada pelo checklist, pela caracterização incompleta e pelo piloto desligado. Nenhuma escrita, documento, Pix, segunda via, sessão de acesso, sincronização de snapshot ou nó do Motor de Fluxos pode ser ativado como consequência dessa entrega.
