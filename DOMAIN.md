...
`CRIAR_ORDEM_SERVICO` é risco alto. A versão guarda somente assunto, descrição e `confirmacaoExplicita: true`; fila, cliente, contrato e protocolo oficial são derivados do atendimento. Antes de conceder e novamente antes de confirmar, o serviço exige atendimento `AGUARDANDO/BOT`, sem responsável, execução `EXECUTANDO` da mesma versão, vínculo automatizável verificado, contrato ativo no contexto, fila exata e protocolo oficial. A auditoria usa origem `FLUXO`, sem usuário ou sessão fabricados. A mesma execução+nó representa uma única OS, inclusive em repetição e reconciliação.
