...
export function AdministracaoOperacionalWeb() {
  ...
  return <main className="pagina-shell administracao-operacional">
    ...
    {estado === 'PRONTO' && painel !== undefined && <div className="grade-operacional">
      <section className="painel-operacional painel-integracoes"><header><strong>Integrações</strong><small>Estado observado</small></header><div className="grade-integracoes">{painel.integracoes.map((integracao) => <article key={integracao.codigo}><i className={classeEstadoIntegracao(integracao.estado)} /><div><strong>{rotuloIntegracao(integracao.codigo)}</strong><small>{integracao.detalhe}</small></div><span>{rotuloEstadoIntegracao(integracao.estado)}</span></article>)}</div></section>
...
function classeEstadoIntegracao(estado: string): string { return estado === 'ATIVA' ? 'integracao-ativa' : estado === 'PARCIAL' ? 'integracao-parcial' : ''; }
function rotuloEstadoIntegracao(estado: string): string { return estado === 'ATIVA' ? 'Ativa' : estado === 'PARCIAL' ? 'Parcial' : 'Não configurada'; }
