...
- Observabilidade usa somente labels e campos fechados de baixa cardinalidade. Nunca use rota concreta, UUID, usuário, fila, contato, mensagem, header ou payload em métrica, trace ou alerta. `traceparent` inválido é descartado; backlog expõe apenas contagem/idade e a projeção administrativa exige `ADMINISTRAR_INTEGRACOES`. Saúde pública continua genérica.
- Deploy executa exatamente um job de migration antes do candidato e só recebe tráfego após `PRONTO`. Em sinal, marque drenagem antes de fechar SSE/WebSocket e deixe worker concluir apenas o ciclo adquirido. Reversão usa imagem anterior compatível e jamais desfaz migration, edita `_prisma_migrations` ou altera estado de negócio por SQL.
