...
Na PR 117, a primeira conexão real com o MK Solutions é deliberadamente somente leitura: o adapter normaliza cliente por critério exato observado, contratos, conexões cadastradas e faturas no contexto explícito de cliente+contrato. A única integração exposta ao operador nesta etapa é a consulta financeira do console, sob liberação própria; o controle cadastral fica reservado e sem rota pública. A consulta MK cobre somente um mês, informa essa limitação na interface e não pode ser apresentada como visão integral nem usada pelo Motor de Fluxos. A linha digitável observada pode compor o resultado financeiro parcial; documento, Pix e segunda via permanecem indisponíveis. Nenhuma liberação habilita escrita. Conexão cadastrada significa apenas um cadastro técnico no ERP: não informa presença online, não substitui `AccessSessionAdapter` e não autoriza desconexão. Busca por nome/telefone, snapshot incremental e todas as escritas permanecem indisponíveis até caracterização própria. Staging continua sintético ou sanitizado, e produção permanece bloqueada.
