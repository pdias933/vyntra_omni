...
A revisão PR124A substitui o fundo verde-escuro por grafite neutro e reserva verde para ações, seleção e estados. Na web, navegação compacta, lista limitada e conversa predominante corrigem a proporção; detalhes ficam lado a lado quando há espaço. Em largura compacta, voltar à lista e retornar à mesma conversa conserva rascunho e posição. Conversa recolhida não confirma leitura de mensagens novas. Não recriar funções, fotos, dados ou canais presentes apenas na ilustração.
...
- No app, consultar faturas identifica explicitamente `TEMPO_REAL` ou indisponibilidade, sem trocar para snapshot financeiro. Desbloqueio e criação de ordem de serviço passam por seleção, prévia e confirmação e reutilizam os casos de uso idempotentes do backend. Segunda via, Pix, sessão de acesso e capacidades ainda não conectadas permanecem indisponíveis, sem sucesso simulado.

Na PR 117/117A, a primeira conexão real com o MK Solutions é deliberadamente somente leitura: o adapter normaliza cliente apenas por documento, contratos, conexões cadastradas e faturas abertas ou pagas no contexto explícito de cliente+contrato. A consulta direta de cliente por identificador externo não possui serviço mínimo confirmado e permanece indisponível. A única integração exposta ao operador nesta etapa é a consulta financeira do console, sob liberação própria; o controle cadastral fica reservado e sem rota pública. A consulta MK cobre somente um mês, informa essa limitação na interface e não pode ser apresentada como visão integral nem usada pelo Motor de Fluxos. A linha digitável observada pode compor apenas uma fatura aberta; documento, Pix e segunda via permanecem indisponíveis. Nenhuma liberação habilita escrita. Conexão cadastrada significa apenas um cadastro técnico no ERP: não informa presença online, não substitui `AccessSessionAdapter` e não autoriza desconexão. Busca por nome/telefone, snapshot incremental e todas as escritas permanecem indisponíveis até caracterização própria. Staging continua sintético ou sanitizado, e produção permanece bloqueada.
